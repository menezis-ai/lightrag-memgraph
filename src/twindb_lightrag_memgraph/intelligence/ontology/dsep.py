...
DSEP_OPERATORS: dict[str, DSEPOperator] = {
    "structural_analysis": DSEPOperator(
        ...
        directive="Ignore noise (buzzwords). Find the root entity and its key attributes.",
    ),
    "scope_exclusion": DSEPOperator(
        symbol="\u29b0",
        name="Scope Exclusion",
        directive="Exclude legacy context. Exclude current process biases.",
    ),
    "gap_analysis": DSEPOperator(
        symbol="\u2a0e",
        name="Gap Analysis",
        directive="Identify where the standard model fails. Find the edge case.",
    ),
    "bounded_context": DSEPOperator(
        symbol="\u29c7",
        name="Bounded Context",
        directive="Define this domain (DDD). Nothing enters, nothing exits.",
    ),
    "entity_definition": DSEPOperator(
        symbol="\u2295",
        name="Entity Definition",
        directive="Define the object formally. Type, Properties, Relations.",
    ),
    "convergence": DSEPOperator(
        symbol="\u2af8",
        name="Migration / Mapping",
        directive="Map the old concept (legacy) to the new target model.",
    ),
}
...
def build_dsep_block(operators: list[str], mode: str) -> str:
    ...
    lines.append("")
    lines.append("Apply these constraints to every entity you extract.")
