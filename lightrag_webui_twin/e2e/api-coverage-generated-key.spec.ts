/**
 * Adversarial route reachability for the dynamically discovered API surface,
 * authenticated with a key minted through the Settings API —
 * POST /twin/api/settings/api-keys, the exact endpoint the WebUI "Create API
 * key" button hits. This spec proves the API/auth seam; the MSW Settings
 * journey separately proves the button wiring.
 *
 * This proves the credential lifecycle: a key generated by the Settings
 * endpoint is accepted by ``require_auth``, not a display-only artefact. It
 * intentionally remains non-admin: routes guarded
 ...
 * every reachability request then uses the generated key. The battery refuses
 * REAL_BACKEND_URL unless REAL_E2E_EPHEMERAL=true marks it as disposable.
 */
...
test.describe('Twin API — adversarial route reachability with a Settings-generated key', () => {
  test.skip(!backendUrl, 'Set REAL_BACKEND_URL to run generated-key route reachability.');
  ...
  test('a Settings-generated key authenticates operator routes and is denied admin routes', async () => {
    ...
    test.info().annotations.push({
      type: 'reachability',
      description: `${probed.length} route operations adversarially probed with a Settings-generated key`,
    });
  });
});
