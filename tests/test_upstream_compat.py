...
def test_docstatus_storage_instantiates():
    ...
    # The dedup helpers added by LightRAG 1.5 must be real coroutines.
    assert callable(store.get_doc_by_content_hash)
    assert callable(store.get_doc_by_file_basename)
...
def test_query_param_builder_survives_unknown_fields():
    ...
    param = _make_query_param(
        qp_cls,
        {
            "mode": "mix",
            "top_k": 5,
            "history_turns": 3,  # dropped upstream in 1.5
            "tag_filter": {"all": ["rman"], "any": []},  # Twin extension
        },
    )

    assert param.top_k == 5
    # Whether or not the installed QueryParam declares these as ctor fields,
    # they must be readable on the resulting object.
