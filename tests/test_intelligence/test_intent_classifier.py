...
class TestIntentClassifier:
    ...
    async def test_classify_in_scope(
        self, classifier, mock_openai_client, intent_in_scope_json
    ):
    ...
    async def test_classify_out_of_scope(
        self, classifier, mock_openai_client, intent_oos_json
    ):
    ...
    async def test_classify_greeting(
        self, classifier, mock_openai_client, intent_greeting_json
    ):
    ...
    async def test_classify_malicious(
        self, classifier, mock_openai_client, intent_malicious_json
    ):
    ...
    async def test_classify_escalation(self, classifier, mock_openai_client):
        ...
        escalation_json = json.dumps(
            {"intent": "ESCALATION", "confidence": 0.96, "reason": "P1 incident"}
        )
